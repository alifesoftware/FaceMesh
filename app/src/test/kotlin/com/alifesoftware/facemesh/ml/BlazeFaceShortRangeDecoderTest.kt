...
@Config(sdk = [33])
class BlazeFaceShortRangeDecoderTest {

    @Test
    fun `sigmoid is monotonic and bounded`() {
        val a = BlazeFaceShortRangeDecoder.sigmoid(-100f)
        val b = BlazeFaceShortRangeDecoder.sigmoid(0f)
        val c = BlazeFaceShortRangeDecoder.sigmoid(100f)
    ...
    fun `decode filters out low-confidence anchors`() {
        ...
        val anchors = arrayOf(BlazeFaceShortRangeAnchors.Anchor(0.5f, 0.5f))
        val decoder = BlazeFaceShortRangeDecoder(
            inputSize = 100,
            anchors = anchors,
            scoreThreshold = 0.9f,
        )
        val regs = FloatArray(PipelineConfig.Detector.ShortRange.regStride) // box centred, w/h = 0
    ...
    fun `decode emits a face when score exceeds threshold and box is non-degenerate`() {
        ...
        val regs = FloatArray(PipelineConfig.Detector.ShortRange.regStride).apply {
    ...
    fun `decode performs NMS on heavily overlapping detections`() {
        ...
        val anchors = arrayOf(
            BlazeFaceShortRangeAnchors.Anchor(0.5f, 0.5f),
            BlazeFaceShortRangeAnchors.Anchor(0.5f, 0.5f),
        )
        val decoder = BlazeFaceShortRangeDecoder(
            inputSize = 100,
        ...
        val perAnchor = FloatArray(PipelineConfig.Detector.ShortRange.regStride).apply {
            this[2] = 60f
            this[3] = 60f
        }

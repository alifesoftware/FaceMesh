...
/**
 * Decodes BlazeFace **short-range** raw outputs (regressors + classifications) into
 * [DetectedFace]s. Companion to [BlazeFaceFullRangeDecoder] - the two are deliberately not
 * sharing code; each owns its own anchor type and input-size constant.
 *
 ...
 */
class BlazeFaceShortRangeDecoder(
    private val inputSize: Int = PipelineConfig.Detector.ShortRange.inputSize,
    private val anchors: Array<BlazeFaceShortRangeAnchors.Anchor> = BlazeFaceShortRangeAnchors.FRONT_128,
    ...
    fun decode(
        ...
        require(regressors.size == anchors.size * PipelineConfig.Detector.ShortRange.regStride) {
            "regressors size ${regressors.size} != ${anchors.size * PipelineConfig.Detector.ShortRange.regStride}"
        }
        ...
        for (i in anchors.indices) {
            ...
            val base = i * PipelineConfig.Detector.ShortRange.regStride
    ...
    private fun readLandmark(
        reg: FloatArray,
        offset: Int,
        anchor: BlazeFaceShortRangeAnchors.Anchor,
        scale: Float,
        padX: Float,
        padY: Float,
        srcW: Int,
    ...
    companion object {
        private const val TAG: String = "FaceMesh.Decoder.SR"
        /**
